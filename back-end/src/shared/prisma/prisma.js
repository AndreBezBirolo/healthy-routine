    ...
    query: {
        mealPlan: {
            ...
            async create({ args, query }) {
                const ctx = getRequestContext();
                const result = await query(args);
                const workspaceId = ctx?.workspaceId || result.workspaceId;
                if (workspaceId) {
                    await basePrisma.activityLog.create({
                        data: {
                            workspaceId,
                            ...
                            changes: JSON.stringify({
                                recipeTitle: { old: null, new: result.recipeTitle },
                                mealType: { old: null, new: result.mealType },
                                isSpecial: { old: null, new: result.isSpecial },
                                isMealPrep: { old: null, new: result.isMealPrep },
                            }),
                        },
                    }).catch((err) => {
