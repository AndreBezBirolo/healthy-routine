...
app.setErrorHandler((error, req, reply) => {
  ...
  if (error instanceof ZodError || (error as any)?.issues || (error as any)?.errors) {
    const list = (error as any).issues || (error as any).errors || [];
  ...
  logger.error({ err: error, requestId }, `Unhandled Exception: ${errorMessage}`);
...
app.put('/api/v1/auth/password', { preHandler: [(app as any).authenticate] }, async (req: any, reply) => {
...
app.put('/api/v1/auth/profile', { preHandler: [(app as any).authenticate] }, async (req: any, reply) => {
  const { name } = req.body || {};
...
// --- WORKSPACE ROUTES ---
app.post('/api/v1/workspaces', { preHandler: [(app as any).authenticate] }, async (req: any, reply) => {
  const body = createWorkspaceSchema.parse(req.body);
  const workspace = await workspaceUseCase.createWorkspace(req.user.id, body);
  return reply.status(201).send(workspace);
});

app.get('/api/v1/workspaces', { preHandler: [(app as any).authenticate] }, async (req: any, reply) => {
  const workspaces = await workspaceUseCase.getUserWorkspaces(req.user.id);
  return reply.send(workspaces);
});

app.post('/api/v1/workspaces/join', { preHandler: [(app as any).authenticate] }, async (req: any, reply) => {
  const body = joinWorkspaceSchema.parse(req.body);
  const workspace = await workspaceUseCase.joinWorkspace(req.user.id, body);
  return reply.send(workspace);
});

app.get('/api/v1/workspaces/:workspaceId/members', { preHandler: [(app as any).authenticate] }, async (req: any, reply) => {
...
app.get('/api/v1/workspaces/:workspaceId/meals/today', { preHandler: [(app as any).authenticate] }, async (req: any, reply) => {
  const { workspaceId } = req.params;
...
app.get('/api/v1/workspaces/:workspaceId/meals/week', { preHandler: [(app as any).authenticate] }, async (req: any, reply) => {
  const { workspaceId } = req.params;
...
app.post('/api/v1/workspaces/:workspaceId/meals', { preHandler: [(app as any).authenticate] }, async (req: any, reply) => {
  const { workspaceId } = req.params;
  const store = requestContext.getStore();
...
app.post('/api/v1/workspaces/:workspaceId/meals/batch', { preHandler: [(app as any).authenticate] }, async (req: any, reply) => {
  const { workspaceId } = req.params;
  const store = requestContext.getStore();
  if (store) store.workspaceId = workspaceId;
...
app.put('/api/v1/workspaces/:workspaceId/meals/:mealId', { preHandler: [(app as any).authenticate] }, async (req: any, reply) => {
...
app.patch('/api/v1/workspaces/:workspaceId/meals/:mealId/toggle-consumed', { preHandler: [(app as any).authenticate] }, async (req: any, reply) => {
  const { mealId } = req.params;
...
app.delete('/api/v1/workspaces/:workspaceId/meals/:mealId', { preHandler: [(app as any).authenticate] }, async (req: any, reply) => {
  const { mealId } = req.params;
...
app.get('/api/v1/workspaces/:workspaceId/tasks', { preHandler: [(app as any).authenticate] }, async (req: any, reply) => {
  const { workspaceId } = req.params;
  const tasks = await taskUseCase.getTasks(workspaceId);
  return reply.send(tasks);
});

app.post('/api/v1/workspaces/:workspaceId/tasks', { preHandler: [(app as any).authenticate] }, async (req: any, reply) => {
  const { workspaceId } = req.params;
  const store = requestContext.getStore();
  if (store) store.workspaceId = workspaceId;
...
app.put('/api/v1/workspaces/:workspaceId/tasks/:taskId', { preHandler: [(app as any).authenticate] }, async (req: any, reply) => {
...
app.patch('/api/v1/workspaces/:workspaceId/tasks/:taskId/toggle', { preHandler: [(app as any).authenticate] }, async (req: any, reply) => {
  const { taskId } = req.params;
...
app.delete('/api/v1/workspaces/:workspaceId/tasks/:taskId', { preHandler: [(app as any).authenticate] }, async (req: any, reply) => {
  const { taskId } = req.params;
...
app.get('/api/v1/workspaces/:workspaceId/recipes', { preHandler: [(app as any).authenticate] }, async (req: any, reply) => {
  const { workspaceId } = req.params;
...
app.post('/api/v1/workspaces/:workspaceId/recipes', { preHandler: [(app as any).authenticate] }, async (req: any, reply) => {
  const { workspaceId } = req.params;
  const store = requestContext.getStore();
  if (store) store.workspaceId = workspaceId;
...
app.get('/api/v1/workspaces/:workspaceId/recipes/roulette', { preHandler: [(app as any).authenticate] }, async (req: any, reply) => {
  const { workspaceId } = req.params;
...
app.post('/api/v1/workspaces/:workspaceId/recipes/ai-suggest', { preHandler: [(app as any).authenticate] }, async (req: any, reply) => {
...
app.get('/api/v1/workspaces/:workspaceId/expenses', { preHandler: [(app as any).authenticate] }, async (req: any, reply) => {
  const { workspaceId } = req.params;
  const expenses = await expenseUseCase.getExpenses(workspaceId);
  return reply.send(expenses);
});

app.post('/api/v1/workspaces/:workspaceId/expenses', { preHandler: [(app as any).authenticate] }, async (req: any, reply) => {
  const { workspaceId } = req.params;
  const store = requestContext.getStore();
  if (store) store.workspaceId = workspaceId;
...
app.get('/api/v1/workspaces/:workspaceId/expenses/balance', { preHandler: [(app as any).authenticate] }, async (req: any, reply) => {
  const { workspaceId } = req.params;
...
app.get('/api/v1/workspaces/:workspaceId/shopping', { preHandler: [(app as any).authenticate] }, async (req: any, reply) => {
  const { workspaceId } = req.params;
  const list = await shoppingUseCase.getShoppingList(workspaceId);
  return reply.send(list);
});

app.post('/api/v1/workspaces/:workspaceId/shopping', { preHandler: [(app as any).authenticate] }, async (req: any, reply) => {
  const { workspaceId } = req.params;
  const store = requestContext.getStore();
  if (store) store.workspaceId = workspaceId;
...
app.put('/api/v1/workspaces/:workspaceId/shopping/:itemId', { preHandler: [(app as any).authenticate] }, async (req: any, reply) => {
...
app.patch('/api/v1/workspaces/:workspaceId/shopping/:itemId/toggle', { preHandler: [(app as any).authenticate] }, async (req: any, reply) => {
  const { itemId } = req.params;
...
app.delete('/api/v1/workspaces/:workspaceId/shopping/:itemId', { preHandler: [(app as any).authenticate] }, async (req: any, reply) => {
  const { itemId } = req.params;
  const result = await shoppingUseCase.deleteItem(itemId);
  return reply.send(result);
});

app.delete('/api/v1/workspaces/:workspaceId/shopping/clear-checked', { preHandler: [(app as any).authenticate] }, async (req: any, reply) => {
  const { workspaceId } = req.params;
...
app.get('/api/v1/workspaces/:workspaceId/billing/status', { preHandler: [(app as any).authenticate] }, async (req: any, reply) => {
  const { workspaceId } = req.params;
  const status = await billingUseCase.getSubscriptionStatus(workspaceId);
  return reply.send(status);
});

app.post('/api/v1/workspaces/:workspaceId/billing/checkout', { preHandler: [(app as any).authenticate] }, async (req: any, reply) => {
  const { workspaceId } = req.params;
...
app.get('/api/v1/workspaces/:workspaceId/activity', { preHandler: [(app as any).authenticate] }, async (req: any, reply) => {
  const { workspaceId } = req.params;
