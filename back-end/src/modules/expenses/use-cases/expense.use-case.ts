...
import { CreateExpenseInput } from '../dtos/expense.dto.js';
...
export class ExpenseUseCase {
  ...
  async createExpense(workspaceId: string, userId: string, data: CreateExpenseInput) {
  ...
  async getBalanceSummary(workspaceId: string) {
    ...
    const userTotals: Record<string, { name: string; paid: number }> = {};
    members.forEach((m) => {
